/**
 * import-n2-content.ts
 *
 * Imports JLPT N2 content from JSON files in scripts/n2-data/ into local Supabase.
 *
 * Usage:
 *   npx tsx scripts/import-n2-content.ts
 *
 * Prerequisites:
 *   - Local Supabase running (npx supabase start)
 *   - JSON data files in scripts/n2-data/
 */
...
function readJsonFile<T>(filename: string): T {
  const filepath = path.resolve(__dirname_compat, 'n2-data', filename);
...
const importSteps: ImportStep[] = [
  ...
  // 2. Lessons
  {
    ...
    skipCheck: async (sb) => {
      const { count } = await sb
        ...
        .in('chapter_id', [
          'e56442ee-3c13-4009-8e18-9ee0470fac29',
          '0fc145f8-da02-4b31-8fba-e692c7d07aaa',
          'b7775f4b-faa7-4f73-b936-df294038d2bf',
          'b5a8b4b6-c76d-4293-893e-858354401cf3',
          '3b403076-383c-4541-9ac5-4555b859b1b6',
          'c0abd299-f7a1-4f5a-bb29-c5552e8b4402',
        ]);
      return (count || 0) > 0;
    },
  },
  ...
  // 4. Quiz Questions
  {
    ...
    skipCheck: async (sb) => {
      // Check if any quiz questions exist for N2 lessons
      const { count } = await sb
        .from('quiz_questions')
        .select('id', { count: 'exact', head: true })
        .in('lesson_id', [
  ...
  // 13. Listening Questions
  {
    ...
    skipCheck: async (sb) => {
      const { count } = await sb
        ...
        .select('id', { count: 'exact', head: true })
        .like('listening_id', 'listening-n2-%');
      return (count || 0) > 0;
    },
  },
  // 14. Speaking Lessons
...
async function main() {
  console.log('🚀 JLPT N2 Content Import\n');
  ...
  for (const step of importSteps) {
    ...
    if (exists) {
      console.log(`⏭  already has N2 data, skipping (${rows.length} rows)`);
